...
class SqlitePersistenceIntegrationTest {
    ...
    void shouldExportToJsonAndCsv() throws Exception {
        ...
        Path outputDir = config.getOutputFile().getParent();
        Path jsonPath = config.getOutputFile();
        String csvName = jsonPath.getFileName().toString().replace(".json", ".csv");
        assertThat(jsonPath).exists();
        assertThat(jsonPath.resolveSibling(csvName)).exists();
    }
}